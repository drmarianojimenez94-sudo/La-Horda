...
|---|---|---|
| **ONBOARDING** | **PASS WITH ISSUES** | Existe un tutorial jugable sin pantallas: el Hechicero enseña moverse → atacar → habilidad y después revivir, runas, emboscadas, fisuras, frío y braseros cuando aparecen. Lo aprendido se guarda y no se repite (`TUT.*` 4/4). **Falta:** que lo pruebe una persona nueva (claridad del texto y del ritmo). De los 18 conceptos del documento cubre 7: movimiento, ataque, habilidad, acción contextual, aliados/revivir, aviso (emboscada) e identidad de arena. Le faltan targeting, cooldown, recurso, daño recibido, jerarquía enemiga, XP, level-up, mejora de habilidad, buff y los avisos de jefe. El retrato del Hechicero es provisorio. |
