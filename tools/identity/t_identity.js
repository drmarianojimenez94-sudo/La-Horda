...
(async () => {
  ...
  if (want('lab')) {
    ...
    await E(() => { updateAllies = __ua; });
    const bot = await E(() => { __start('laberinto', 3); __calm(); LAB.spawnT = 10; __step(100); player.x = LAB.seals[0].x - 100; player.y = LAB.seals[0].y; let t = 0; const order = []; const trace = []; while (LAB.seals.length && t < 90000){ __step(250); t += 250; enemies.length = 0; for (const s of LAB.seals) if (s.lit && !order.includes(s.n)) order.push(s.n); if (t % 5000 === 0){ const g = allies.find(h => h._ctxGoal); const tg = g && LAB.seals.find(s => s.id === g._ctxGoal); trace.push(g ? `${g.classKey}@${g.x|0},${g.y|0}>${tg ? tg.n+'@'+tg.x+','+tg.y : '?'} un${g._ctxUnstick|0} nx${LAB.next}` : 'nogoal nx' + LAB.next); } } return { solved: LAB.solved > 0, order: order.join(''), t: t/1000, wrong: LAB.wrongs, trace: LAB.solved ? null : trace }; });
