...
(async () => {
  ...
  await p.click('#title-continue-btn'); await p.click('#mainmenu-jugar-btn'); await p.click('#mode-arena-btn');
  const cards = await p.$$eval('.arena-card:not(.divina)', els => els.map(e => e.dataset.arena + ':' + (e.disabled ? 'locked' : 'open')));
  check('new_save.only_first_arena_open', cards.join(',') === 'bosque:open,acuatica:locked,hielo:locked,laberinto:locked,infernal:locked', cards);
  ...
  check('progression.bosque_opens_acuatica', after.slice(0, 3).join(',') === 'bosque:open,acuatica:open,hielo:locked', after);
