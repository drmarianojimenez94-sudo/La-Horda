...
(async () => {
  ...
  const mk = async (url, i) => {
    ...
    await p.evaluate(([c]) => { for (const k in save.champions) { save.champions[k].level = 30; save.champions[k].unlocked = true; save.champions[k].skillMastery.forEach(m => m.alloc = 5); save.champions[k].ultMastery.alloc = 3; } save.arenasCleared = { bosque: true, acuatica: true, hielo: true, laberinto: true, infernal: true }; selectedClass = c; }, [CH[i]]);
    return { p, errs, i };
  };
